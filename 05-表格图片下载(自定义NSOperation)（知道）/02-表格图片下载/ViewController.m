...
- (void)downloadImage:(NSIndexPath *)indexPath
{
    ...
    // 没有下载操作，创建自定义的下载操作，来下载图片
    CZDownloadOperation *downloadOp = [[CZDownloadOperation alloc]init];
    
    // 设置url
    downloadOp.urlString = app.icon;
    
      // void (^completionBlock)(void)
    // 操作执行结束的回调方法
    __weak CZDownloadOperation *weakOp = downloadOp;
